...
export class SocialDataService {
  ...
  fetchForums(){
    this.http.get<Forum[]>('http://localhost:8000/ProyectoFinal_DAW-main/HTMLRequests/getForums.php')
     .subscribe(response => {
  ...
  fetchPosts(forum_id: number){
    this.http.get<Post[]>('http://localhost:8000/ProyectoFinal_DAW-main/HTMLRequests/getPosts.php', {params: {'forum_id': forum_id}})
     .subscribe(response => {
  ...
  fetchComments(post_id: number){
    this.http.get<Comment[]>('http://localhost:8000/ProyectoFinal_DAW-main/HTMLRequests/getComments.php', {params: {'post_id': post_id}})
     .subscribe(response => {
  ...
  postComment(user_id: number, post_id: number, content: string){
    this.http.get<Comment[]>('http://localhost:8000/ProyectoFinal_DAW-main/HTMLRequests/postComment.php', {params: {'user_id': user_id, 'post_id': post_id, 'content': content}})
    .subscribe(response => {
  ...
  postPost(forum_id:number, user_id: number,title: string, content: string, image?: File){
    this.http.get<Post[]>('http://localhost:8000/ProyectoFinal_DAW-main/HTMLRequests/postPost.php', {params: {'forum_id':forum_id, 'user_id':user_id,'title': title, 'content': content, 'has_image': image != null ? 1: 0}})
    .subscribe(response => {
      console.log(response);
      if(!image){
  ...
  postForum(name: string, description: string, image: File){
    this.http.get<Post[]>('http://localhost:8000/ProyectoFinal_DAW-main/HTMLRequests/postForum.php', {params: {'forum_name':name, 'forum_description':description}})
    .subscribe(response => {
      console.log(response);
