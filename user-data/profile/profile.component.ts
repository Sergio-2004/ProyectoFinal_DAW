...
export class ProfileComponent {
  ...
  uploadDescription(description: string){
    ...
    this.http.get('http://localhost:8000/ProyectoFinal_DAW-main/HTMLRequests/uploadDescription.php', {params: {'user_id': this.sessionService.getSession()!.id, 'description': description}})
      .subscribe({
        next: (response) => {
          console.log('Description uploaded successfully:', response);
          this.http.get<any>('http://localhost:8000/ProyectoFinal_DAW-main/HTMLRequests/getUserData.php',  { params: { "user_id": this.sessionService.getSession()!.id}})
        .subscribe((response) => {
  ...
  deleteUser(){
    this.http.get<any>('http://localhost:8000/ProyectoFinal_DAW-main/HTMLRequests/deleteUser.php', {params: {'user_id': this.sessionService.getSession()!.id}})
      .subscribe(response => {
